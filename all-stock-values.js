...
exports.GetAllStockValues = async function() {
    ...
    const columnNames = await page.evaluate(()=>{
        const all = document.querySelectorAll(".cell-seAzPAHn");
        let list = [];
        all.forEach(element => {
...
async function autoScroll(page,num){
    await delay(700);

    const scrollable_section = '#js-screener-container > div > div > div.content-VHbjaC7h.sidenavOpen-VHbjaC7h > div > div.tableContainer-tmg4POBa > div > div > div.wrapper-yDuvR4fK > div > table > tbody ';

    await page.waitForSelector(scrollable_section);
    
    for (let index = 0; index < num; index++) {
        await page.evaluate(selector => {
            document.querySelector(selector).lastChild.scrollIntoView();
        }, scrollable_section);
        await delay(1700);
    }
}
